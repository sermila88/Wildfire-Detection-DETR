...
# CONFIGURATION
# ============================================================================
EXPERIMENT_NAME = "rf_detr_hyperparameter_tuning_v4.1"
        ...
        while True:
            try:
                ...
                if hp['batch_size'] > 2:
                    hp['batch_size'] //= 2
                    hp['grad_accum_steps'] = max(1, EFFECTIVE_BATCH_SIZE // hp['batch_size'])
                    print(f"    → Retrying with batch_size={hp['batch_size']} (accum={hp['grad_accum_steps']})")
                elif hp['resolution'] == 1120:
                    hp['resolution'] = 896
                    print("    → Retrying with resolution=896")
                    # re-init model at the new resolution & 1 class
                    del model
                    torch.cuda.empty_cache()
                    model = RFDETRBase(resolution=hp['resolution'], num_classes=1)
                else:
                    print("    → Cannot shrink further; pruning trial.")
