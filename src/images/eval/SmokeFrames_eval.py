"""
Evaluation Script for SmokeFrames
"""
...
# Configuration
OUTPUT_BASE_DIR = "/vol/bitbucket/si324/rf-detr-wildfire/src/images/SmokeFrames_eval"
TEST_IMAGES_DIR = "/vol/bitbucket/si324/rf-detr-wildfire/src/images/data/SmokeFrames/images/test"
GT_FOLDER = "/vol/bitbucket/si324/rf-detr-wildfire/src/images/data/SmokeFrames/labels/test"
...
def create_comparison_visualizations(all_model_results, all_results_data, output_dir):
    ...
    metrics = ['F1 Score', 'Precision', 'Recall', 'Accuracy']  # Add Accuracy
    metric_keys = ['best_f1', 'precision', 'recall', 'accuracy']  # Add accuracy
    color_keys = ['f1', 'precision', 'recall', 'accuracy']  # Add accuracy
    x = np.arange(len(model_order))
    width = 0.20  # Changed from 0.25 to 0.20 to fit 4 bars
    
    # Add accuracy color if not already there
    if 'accuracy' not in bar_colors:
        bar_colors['accuracy'] = '#9B59B6'  # Purple for Accuracy
    
    for i, metric in enumerate(metric_keys):
        values = [all_model_results[m]['image_level'][metric] for m in model_order]
        positions = x + (i - 1.5) * width  # Changed to center 4 bars
        color = bar_colors[color_keys[i]]
        bars = ax.bar(positions, values, width, label=metrics[i], color=color)
        for j, bar in enumerate(bars):
