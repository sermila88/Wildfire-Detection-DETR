"""
NEMO Evaluation Script for Wildfire Smoke Detection
Evaluate on NEMO validation set
"""
...
# Configuration
NEMO_VAL_DIR = "/vol/bitbucket/si324/rf-detr-wildfire/src/images/data/nemo_val"
OUTPUT_BASE_DIR = "/vol/bitbucket/si324/rf-detr-wildfire/src/images/eval_results_NEMO"
TEST_IMAGES_DIR = os.path.join(NEMO_VAL_DIR, "img")

def convert_nemo_annotations_to_yolo(nemo_val_dir, output_dir):
    """Convert NEMO format to YOLO format for evaluation - ONE box per image"""
    import json
    from pathlib import Path
    import cv2
    
    ann_dir = Path(nemo_val_dir) / "ann"
    img_dir = Path(nemo_val_dir) / "img"
    labels_dir = Path(output_dir) / "labels"
    labels_dir.mkdir(parents=True, exist_ok=True)
    
    # Track statistics
    stats = {"low": 0, "mid": 0, "high": 0, "empty": 0}
    
    # Process each annotation file
    for ann_file in ann_dir.glob("*.jpg.json"):
        ...
        label_file = labels_dir / f"{label_name}.txt"
        ...
        # Collect boxes by smoke intensity
        ...
        for obj in data["objects"]:
            ...
            if "smoke" in class_title:
                ...
                x1, y1 = points[0]
                x2, y2 = points[1]
                # Calculate box area for later selection
                area = (x2 - x1) * (y2 - y1)
                box = [x1, y1, x2, y2, area]
                
                if "low" in class_title:
                    low_smoke_boxes.append(box)
                elif "mid" in class_title:
                    mid_smoke_boxes.append(box)
        ...
        # Write single box to file
        with open(label_file, 'w') as f:
            f.write(f"0 {center_x:.6f} {center_y:.6f} {width:.6f} {height:.6f}")
        
        stats[selected_type] += 1
    
    # Report statistics
    total_files = len(list(labels_dir.glob('*.txt')))
    print(f"\nConverted {total_files} NEMO annotations to YOLO format")
    print(f"Smoke distribution (ONE box per image):")
    print(f"  Low smoke: {stats['low']} images")
    print(f"  Mid smoke: {stats['mid']} images")
    print(f"  High smoke: {stats['high']} images")
    print(f"  No smoke: {stats['empty']} images")
    print(f"  Total with smoke: {stats['low'] + stats['mid'] + stats['high']} images")
    ...
    total_files = len(list(labels_dir.glob('*.txt')))
    print(f"\nConverted {total_files} NEMO annotations to YOLO format")
    print(f"  Total smoke boxes: {stats['total_boxes']}")
    print(f"  Images with smoke: {stats['images_with_smoke']}")
    print(f"  Images without smoke: {stats['empty_images']}")
    print(f"  Avg boxes per image: {stats['total_boxes']/total_files:.2f}")
...
def evaluate_predictions(pred_folder, gt_folder, conf_th=0.1, cat=None):
    """
    NEMO-specific evaluation: Image-level with overlap checking
    - Images with TP: At least one prediction overlaps with ANY GT
    - Images with FP: At least one prediction has NO overlap with any GT
    - Images with FN: At least one GT has NO overlap with any prediction
    """
    ...
    for filename in all_filenames:
        ...
        # Load predictions above threshold
        pred_boxes = []
            ...
            with open(pred_file, "r") as f:
                for line in f.readlines():
                    ...
                    if len(parts) >= 6:
                        _, x, y, w, h, conf = map(float, parts)
                        if conf >= conf_th:
                            pred_boxes.append(xywh2xyxy(np.array([x, y, w, h])))
        
        # Track which boxes are matched (WITHOUT one-to-one constraint)
        gt_matched = [False] * len(gt_boxes)
        pred_matched = [False] * len(pred_boxes)
        
        # Match ALL predictions to GT boxes
        for pred_idx, pred_box in enumerate(pred_boxes):
            for gt_idx, gt_box in enumerate(gt_boxes):
                iou = box_iou(pred_box, gt_box)
                if iou > 0.01:  # Any overlap counts as match
                    pred_matched[pred_idx] = True
                    gt_matched[gt_idx] = True  # GT can be matched multiple times
                    break  # This prediction is matched, move to next
        
        # Count image-level results
        has_tp = any(pred_matched)  
        has_fp = any(not matched for matched in pred_matched) if pred_boxes else False
        has_fn = any(not matched for matched in gt_matched) if gt_boxes else False
        ...
        # Count boxes
        total_tp_boxes += sum(pred_matched)
        total_fp_boxes += len(pred_boxes) - sum(pred_matched)
        total_fn_boxes += len(gt_boxes) - sum(gt_matched)
    
    # Metrics (no TN for NEMO)
    precision = images_with_tp / (images_with_tp + images_with_fp) if (images_with_tp + images_with_fp) > 0 else 0
    recall = images_with_tp / (images_with_tp + images_with_fn) if (images_with_tp + images_with_fn) > 0 else 0
...
def classify_image_boxes(pred_file, gt_file, conf_threshold):
    """Mirror exact logic from evaluate_predictions for consistency"""
    # Load GT boxes
    gt_boxes = []
    if os.path.isfile(gt_file) and os.path.getsize(gt_file) > 0:
        with open(gt_file, "r") as f:
            ...
            for line in f.readlines():
                ...
                if len(parts) >= 6:
                    ...
                    if conf >= conf_threshold:
                        ...
                        pred_data.append({'box': pred_box, 'conf': conf, 'type': None, 'iou': 0.0})
    
    # Track which boxes are matched
    gt_matched = [False] * len(gt_boxes)
    
    # Match predictions to GT boxes - WITHOUT one-to-one constraint
    for pred in pred_data:
        ...
        for gt_idx, gt_box in enumerate(gt_boxes):
            ...
            if iou > best_iou:
                best_iou = iou
            if iou > 0.01:  # Any overlap counts
                matched = True
                gt_matched[gt_idx] = True  # GT can be matched by multiple preds
                # Don't break - continue to find best IoU for display
        
        pred['iou'] = best_iou
        pred['type'] = 'TP' if matched else 'FP'
...
def find_best_conf_threshold_and_plot(
    ...
    if plot:
        ...
        plt.tight_layout()
        plt.savefig(os.path.join(pred_folder, "metrics.png"), bbox_inches='tight', dpi=150)
        # save the list
...
def create_comparison_visualizations(all_model_results, all_results_data, output_dir):
    ...
    model_order = ['YOLO_baseline', 'RT-DETR_HP_best', 'RF-DETR_initial_training_NMS']
...
if __name__ == "__main__":
    ...
    # Convert NEMO annotations to YOLO format
    print("Converting NEMO annotations to YOLO format...")
    CONVERTED_LABELS_DIR = os.path.join(OUTPUT_BASE_DIR, "nemo_converted_labels")
    os.makedirs(CONVERTED_LABELS_DIR, exist_ok=True)
    GT_FOLDER = convert_nemo_annotations_to_yolo(NEMO_VAL_DIR, CONVERTED_LABELS_DIR)
    print(f"Ground truth labels saved to: {GT_FOLDER}")
    
    # Verify conversion
    num_gt_files = len(glob.glob(os.path.join(GT_FOLDER, "*.txt")))
    num_images = len(glob.glob(os.path.join(TEST_IMAGES_DIR, "*.jpg")))
    print(f"Found {num_gt_files} ground truth files for {num_images} images")
