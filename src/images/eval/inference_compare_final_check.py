...
# Configuration
OUTPUT_BASE_DIR = "/vol/bitbucket/si324/rf-detr-wildfire/src/images/final_inference_compare_GPU"
...
def evaluate_model(model_name, model_type, model_path, conf_thres_range):
    ...
    if model_type == "YOLO":
        ...
        yolo_total_time = time.time() - yolo_start
        # Estimate per-image time (YOLO processes all at once)
        num_test_images = len(glob.glob(os.path.join(TEST_IMAGES_DIR, "*.jpg"))) + len(glob.glob(os.path.join(TEST_IMAGES_DIR, "*.png")))
        inference_times = [yolo_total_time / num_test_images * 1000] * num_test_images  # Convert to ms
    else:
    ...
    if inference_times:
        ...
        fps = 1000 / mean_time_ms if mean_time_ms > 0 else 0
        
        print(f"\nInference Speed Statistics:")
        print(f"  Mean time per frame: {mean_time_ms:.2f} ms")
    ...
    return best_obj_results, best_obj_conf, best_img_results, best_img_conf, mean_time_ms, fps
...
def create_object_confusion_matrix(best_results, model_name, plots_dir):
    ...
    sns.heatmap(cm_numeric, annot=cm_labels, fmt="", 
                ...
                square=True)
    #plt.title(f"{model_name} – Confusion Matrix (IoU=0.01)", fontsize=10)
    plt.tight_layout() 
...
if __name__ == "__main__":
    ...
    for model_name, config in models.items():
        best_obj_results, best_obj_conf, best_img_results, best_img_conf, mean_time_ms, fps = evaluate_model(
            model_name, 
            config['type'],
            config['path'],
            conf_range
    ...
    with open(os.path.join(OUTPUT_BASE_DIR, "final_comparison_summary.txt"), 'w') as f:
        ...
        for model_name, results in all_model_results.items():
            if 'speed' in results:
                f.write(f"{model_name}:\n")
                f.write(f"  Mean time: {results['speed']['mean_ms']:.2f} ms\n")
                f.write(f"  FPS: {results['speed']['fps']:.2f}\n\n")
    ...
    for model_name, results in all_model_results.items():
        if 'speed' in results:
            print(f"{model_name}:")
            print(f"  Mean time: {results['speed']['mean_ms']:.2f} ms")
