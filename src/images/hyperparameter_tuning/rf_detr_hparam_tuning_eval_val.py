"""
RF-DETR Hyperparameter Tuning Re-evaluation with NMS
"""
...
# CONFIGURATION
# ============================================================================
EXPERIMENT_DIR = "/vol/bitbucket/si324/rf-detr-wildfire/src/images/outputs/RF-DETR_hyperparameter_tuning"
...
def reevaluate_trial(trial_num, trial_dir, val_dataset):
    ...
    print(f"  📊 Resolution: {hyperparams.get('resolution', 896)}")
    print(f"  🎯 Batch size: {hyperparams.get('batch_size', 8)}")
    print(f"  📈 LR: {hyperparams.get('lr', 1e-4):.2e}")
    
    try:
        model = RFDETRBase(
            pretrain_weights=checkpoint_path,
            num_classes=1,
            resolution=hyperparams.get('resolution', 896)
        )
...
def main():
    print("\n" + "="*60)
    print("RF-DETR HYPERPARAMETER RE-EVALUATION WITH NMS")
    print("="*60)
    ...
    if all_results:
        ...
        with open(os.path.join(OUTPUT_DIR, "final_summary.txt"), 'w') as f:
            f.write("RF-DETR HYPERPARAMETER TUNING FINAL RESULTS (WITH NMS)\n")
            f.write("="*60 + "\n\n")
