"""
RT-DETR Predicted Bounding Box Visualizer with TP/FP/FN/TN Breakdown
Mirrors the RT-DETR evaluation logic exactly for consistent visualization
"""
...
EXPERIMENT_NAME = "rtdetr_smoke_detection_v1_IoU=0.01"  # Update to match eval
...
# Model checkpoint
CHECKPOINTS_DIR = f"{PROJECT_ROOT}/outputs/{EXPERIMENT_NAME}/checkpoints/weights"
...
# Output directories
OUTPUT_ROOT = f"{PROJECT_ROOT}/bounding_boxes/{EXPERIMENT_NAME}/rtdetr_predicted_bb_breakdown"
...
def generate_visualizations():
    ...
    print(f"🎯 RT-DETR Bounding Box Visualizer")
    ...
    # Load model
    checkpoint_path = f"{CHECKPOINTS_DIR}/best.pt"
    ...
    for image_path, annotations in tqdm(combined_dataset, desc="Generating visualizations"):
        ...
        with Image.open(image_path) as img:
            ...
            results = model.predict(source=img_rgb, conf=IMG_CONF_THRESHOLD, verbose=False)
            img_predictions = sv.Detections.from_ultralytics(results[0])
        ...
        with Image.open(image_path) as img:
            img_rgb = img.convert("RGB")
            results = model.predict(source=img_rgb, conf=OBJ_CONF_THRESHOLD, verbose=False)
            obj_predictions = sv.Detections.from_ultralytics(results[0])
